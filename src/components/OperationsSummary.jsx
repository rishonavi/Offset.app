...
//
// They answer the period differently, and the difference is worth keeping. Stock
// is dated — every movement carries a date, so opening and closing values over a
// range are real figures. Payroll is not: Offset holds today's employees and
// today's salaries and no history of past runs, so a past month is computed from
// the payroll as it stands now. The card says so rather than letting a projection
// pass for a record.
...
export function useOperationsSummary(filters) {
  ...
  return useMemo(() => {
    ...
    if (items.length === 0 && employees.length === 0) return null
...
export default function OperationsSummary({ summary }) {
  ...
  const { stock, payroll, periods, ranged, clamped, itemCount, entity } = summary
  const hasStock = itemCount > 0
  const hasPayroll = payroll.months.some((m) => m.headcount > 0)
  if (!hasStock && !hasPayroll) return null
    ...
    <Card className="p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-ink-3">What the company cost</h2>
        </div>
        <span className="text-xs text-ink-6">{entity?.name}</span>
      </div>
      <p className="mt-1 text-xs text-ink-5">
        Stock and payroll for {ranged ? 'the period above' : 'this month'}, alongside the rents and bills.
      </p>
